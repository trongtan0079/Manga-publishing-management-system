 ...
 * @var string $submissionType Loại nộp bài (draft hoặc final)
 */
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h2 class="h3 mb-1" id="form-title">
            <?= $submissionType === 'draft' ? 'Nộp Bản thảo Phác thảo (Draft Storyboard)' : 'Nộp Bản thảo Hoàn chỉnh (Final Manuscript)' ?>
        </h2>
        <p class="text-muted text-xs mb-0">
            <?= $submissionType === 'draft' 
                ? 'Nộp bản thảo kịch bản phác thảo thô (Storyboard) để Biên tập viên duyệt trước khi bắt đầu vẽ chi tiết.' 
                : 'Nộp bản thảo hoàn chỉnh (đầy đủ các trang vẽ hoàn thiện) để Biên tập viên duyệt phát hành/xuất bản.' ?>
        </p>
    </div>
    ...
    <div class="col-lg-8 col-md-10 mx-auto">
        <div class="card shadow-sm border-0 rounded-3">
            <div class="card-header bg-white text-dark py-3 border-bottom border-light">
                <h5 class="card-title mb-0" id="card-title-text">
                    <i class="fas <?= $submissionType === 'draft' ? 'fa-file-alt' : 'fa-file-archive' ?> me-2 text-primary"></i>
                    <?= $submissionType === 'draft' ? 'Thông tin nộp Bản thảo thô (Storyboard / Name)' : 'Thông tin nộp Bản vẽ hoàn chỉnh (Final Manuscript)' ?>
                </h5>
            </div>
            <div class="card-body p-4">
                ...
                <form action="<?= BASE_PATH ?>/index.php?controller=submission&action=store" method="POST" enctype="multipart/form-data">
                        ...
                        <select class="form-select" id="chapter_id" name="chapter_id" required>
                            ...
                            <?php if (!empty($chapters)): ?>
                                ...
                                <?php foreach ($chapters as $c): ?>
                                    <option value="<?= $c['chapter_id'] ?>" data-status="<?= htmlspecialchars($c['status']) ?>" <?= ($c['chapter_id'] == $selectedChapterId) ? 'selected' : '' ?>>
                                        <?= htmlspecialchars($c['series_title']) ?> - Ch.<?= htmlspecialchars($c['chapter_number']) ?>: <?= htmlspecialchars($c['title'] ?? 'Chưa đặt tên') ?> (<?= $c['status'] === 'drafting' ? 'Phác thảo Kịch bản' : 'Đang vẽ Chi tiết' ?>)
                                    </option>
                                <?php endforeach; ?>
                            <?php else: ?>
                    ...
                    <!-- Upload File -->
                    <div class="mb-4">
                        <label for="file" class="form-label fw-bold text-dark" id="file-label">
                            <?php if ($submissionType === 'draft'): ?>
                                <i class="fas fa-file-alt me-2 text-muted"></i>Tải lên bản phác thảo / Kịch bản thô (Storyboard / Name) <span class="text-danger">*</span>
                            <?php else: ?>
                                <i class="fas fa-file-archive me-2 text-muted"></i>Tải lên bản vẽ hoàn chỉnh (Final Manuscript) <span class="text-danger">*</span>
                            <?php endif; ?>
                        </label>
                        <div class="upload-dropzone position-relative d-flex flex-column align-items-center justify-content-center border border-dashed rounded-3 p-4 bg-light text-center" id="dropzone" style="cursor: pointer; transition: background-color 0.2s, border-color 0.2s; border-width: 2px !important; border-color: #cbd5e1 !important; min-height: 150px;">
                            ...
                            <h6 class="fw-semibold mb-1" id="upload-status-text" style="font-size: 0.9rem;">
                                <?= $submissionType === 'draft'
                                    ? 'Kéo thả file kịch bản phác thảo (.zip, .pdf, .jpg) vào đây hoặc click để chọn'
                                    : 'Kéo thả file bản thảo hoàn chỉnh (.zip, .pdf) vào đây hoặc click để chọn' ?>
                            </h6>
                    ...
                    <div class="d-grid gap-2 d-md-flex justify-content-md-end mt-4">
                        ...
                        <button type="submit" class="btn btn-primary px-5 shadow-sm" style="border-radius: 8px;" id="submit-btn-text">
                            <i class="fas fa-paper-plane me-2"></i>
                            <?= $submissionType === 'draft' ? 'Gửi Duyệt Bản Nháp' : 'Nộp Bản thảo Hoàn chỉnh' ?>
                        </button>
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const submissionType = <?= json_encode($submissionType) ?>;
        ...
        fileInput.addEventListener('change', function() {
            if (fileInput.files && fileInput.files[0]) {
                ...
                if (submissionType === 'draft') {
                    statusText.textContent = "Kéo thả file kịch bản phác thảo (.zip, .pdf, .jpg) vào đây hoặc click để chọn";
                } else {
                    statusText.textContent = "Kéo thả file bản thảo hoàn chỉnh (.zip, .pdf) vào đây hoặc click để chọn";
                }

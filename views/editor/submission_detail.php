        ...
        <div class="card shadow-sm border-0 rounded-3 mb-4">
            ...
            <div class="card-body p-0 bg-light text-center" style="min-height: 400px; display: flex; flex-direction: column;">
                <?php if (!empty($submissionHistory) && count($submissionHistory) > 1): ?>
                    <!-- Thanh chuyển đổi phiên bản giống giao diện chi tiết trang (Page Detail) -->
                    <div class="w-100 bg-white border-bottom py-3 px-4 d-flex align-items-center flex-wrap gap-2 text-start shadow-xs">
                        <span class="text-xs fw-bold text-muted text-uppercase me-2" style="letter-spacing: 0.5px;">
                            <i class="fas fa-history me-1.5 text-secondary"></i>Các bản nộp:
                        </span>
                        <div class="btn-group btn-group-sm shadow-sm" role="group" aria-label="Version Switcher">
                            <?php 
                            ...
                            ?>
                                <input type="radio" 
                                       class="btn-check compare-version-radio" 
                                       name="compare_version_toggle" 
                                       id="btn-version-<?= $hist['submission_id'] ?>" 
                                       value="<?= $hist['submission_id'] ?>"
                ...
                <div class="p-4 flex-grow-1 d-flex flex-column align-items-center justify-content-center w-100">
                    <?php if ($isImage): ?>
                        <!-- Panel xem đơn (Hỗ trợ chuyển đổi nhanh) -->
                ...
                <div class="mt-4 pt-3 border-top w-100 d-flex justify-content-between align-items-center text-start">
                    <div>
                        ...
                        <strong class="text-dark text-xs" id="physicalFileNameLabel"><?= htmlspecialchars(basename($submission['file_url'] ?? '')) ?></strong>
                    </div>
                    <div>
                        <a id="downloadPhysicalFileBtn" href="<?= htmlspecialchars((string)($fileUrl ?? '')) ?>" download class="btn btn-outline-dark btn-sm shadow-sm py-1.5 px-3" style="font-size: 12px; border-radius: 6px;">
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // Version radios switcher listener

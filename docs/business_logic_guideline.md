...
2. Khi toàn bộ các Task nhỏ của trợ lý trên một trang đã được Mangaka duyệt hoàn tất, trang truyện sẽ hiển thị **badge vàng "Chờ nộp bản hoàn chỉnh"** (trạng thái DB vẫn là `drawing` với `old_image_url = 'no_genko'`). **Mangaka** sẽ lấy các lớp/phần vẽ rời rạc của Assistant để hợp nhất (merge) thành trang vẽ hoàn chỉnh cuối cùng bằng phần mềm đồ họa chuyên dụng, rồi tải lên bản vẽ Genko hoàn chỉnh qua form **Chỉnh sửa Trang**. Lúc này hệ thống tự động gọi `syncPageStatus()` để chuyển trang sang trạng thái **Hoàn thành (`approved`)** với badge xanh "Hoàn thành (Chờ gửi Editor)".
